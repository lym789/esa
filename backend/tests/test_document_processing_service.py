...
from app.services.document_processing_service import process_document, reindex_completed_documents, split_text_into_chunks
...
def test_process_document_replaces_existing_chunks_on_reindex(tmp_path):
    db = make_session()
...
def test_reindex_completed_documents_reprocesses_only_completed_documents(tmp_path):
    ...
    completed_chunks = (
        ...
        .filter(DocumentChunk.document_id == completed.id)
